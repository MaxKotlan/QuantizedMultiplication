...
```
- Pass `--simulation-type dot` (or `matmul`) to switch from the default chained multiply walk to a multiply‑accumulate run; products and the running sum are saturated to the map’s representable range to mimic bounded quantized matmul.
